...
oc create ns sigstore-monitoring
...
git clone https://github.com/cooktheryan/db.git
oc create -f ./db
kubectl create secret generic pull-sec -n mysql --from-file=.dockerconfigjson=${HOME}/.docker/config.json --type=kubernetes.io/dockerconfigjson

cat <<EOF >>/tmp/tuf-ns.yaml
apiVersion: v1
kind: Namespace
metadata:
  annotations:
    meta.helm.sh/release-name: trusted-artifact-signer
    meta.helm.sh/release-namespace: sigstore
  labels:
    app.kubernetes.io/managed-by: Helm
    kubernetes.io/metadata.name: tuf
  name: tuf-system
EOF

cat <<EOF >>/tmp/trill-ns.yaml
apiVersion: v1
kind: Namespace
metadata:
  annotations:
  ...
  labels:
    app.kubernetes.io/managed-by: Helm
    kubernetes.io/metadata.name: trillian
  name: trillian-system
EOF

oc create -f /tmp/trill-ns.yaml
oc create -f /tmp/tuf-ns.yaml

oc create -f /tmp/trill-secret.yaml



kubectl create secret generic rh-pull --from-file=.dockerconfigjson=${HOME}/.docker/config.json  --type=kubernetes.io/dockerconfigjson -n tuf-system

oc patch serviceaccount -n tuf-system tuf -p '{"imagePullSecrets": [{"name": "pull-secret"}]}'

oc patch serviceaccount -n tuf-system tuf-secret-copy-job -p '{"imagePullSecrets": [{"name": "pull-secret"},{"name": "rh-pull"}]}'
